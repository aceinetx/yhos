...
BINS = build/yhos.img build/kernel_entry.o build/kernel.o build/void.bin build/boot.bin build/lowlevel.o build/syscall.o build/keyboard.o build/shell.o build/std.o build/test.o build/vfs.o
GIT_COMMIT = $(shell git describe --always)
CFLAGS = -I. -Wall -Wpedantic -Werror -Wextra -DGIT_COMMIT='"$(GIT_COMMIT)"' -DDMEM_ADDR_LIMIT=512 -DDMEM_BEGIN=1600000000
...
build/shell.o: kernel/shell.c kernel/shell.h kernel/syscall.h kernel/std.h kernel/std.c kernel/version.h kernel/lowlevel.h kernel/test.asm kernel/vfs.h
	$(dir_guard)
	$(GCC_COLOR)
	i386-elf-gcc -m32 -ffreestanding -g -c $< -o $@ $(CFLAGS)
	$(RESET_COLOR)
...
build/kernel.o: kernel/kernel.c kernel/syscall.h kernel/types.h kernel/keyboard.h kernel/shell.h kernel/version.h kernel/vfs.h
	$(dir_guard)
	$(GCC_COLOR)
	i386-elf-gcc -m32 -ffreestanding -g -c $< -o $@ $(CFLAGS)
	$(RESET_COLOR)

build/vfs.o: kernel/vfs.c kernel/syscall.h kernel/types.h kernel/version.h kernel/vfs.h
	$(dir_guard)
	$(GCC_COLOR)
	i386-elf-gcc -m32 -ffreestanding -g -c $< -o $@ $(CFLAGS)
	$(RESET_COLOR)
