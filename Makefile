...
CFLAGS = -I. -Wall -Wpedantic -Werror -Wextra -DGIT_COMMIT='"$(GIT_COMMIT)"' -DVFS_SIZE=16000000 -DVFS_MAX_FILE_SIZE=4096 -DVFS_MAX_FILE_NAME=16
...
build/syscall.o: kernel/syscall.c kernel/syscall.h kernel/types.h kernel/lowlevel.h kernel/keyboard.h kernel/std.h
	$(dir_guard)
	$(GCC_COLOR)
	i386-elf-gcc -m32 -ffreestanding -g -c $< -o $@ $(CFLAGS)
	$(RESET_COLOR)
